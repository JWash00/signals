...
/**
 * Test Hacker News Live cron ingestion.
 *
 * Default: HTTP mode — calls the cron route (requires dev server: npm run dev)
 *   node scripts/test-cron-hn-live.mjs
 *
 * Remote:
 *   node scripts/test-cron-hn-live.mjs https://signals-mu.vercel.app
 */
...
const url = `${baseUrl.replace(/\/$/, "")}/api/cron/hn-live`;
